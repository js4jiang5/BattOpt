...
所有使用者在安裝後皆可立即享有 **30 天的 Pro 功能免費試用期**，無需信用卡即可開始。
...
避免與 macOS 原生電池管理衝突：
* 前往 **系統設定 > 電池 > 電池健康度**。
* 點擊 **ⓘ** 圖示，將「**最佳化電池充電**」切換為 **關閉**，若為 macOS 26.4 以上將「**充電上限**」設定為為 **100%**，
...
## 📃 免責聲明
BattOpt 使用底層系統呼叫來管理您的 Mac 電池健康。雖然已在 M1 及較舊的 Intel MacBook 上經過廣泛測試，但本軟體仍以「現狀 (AS IS)」提供，不提供任何形式的保證，也不保證能支持未來的 macOS 版本。
